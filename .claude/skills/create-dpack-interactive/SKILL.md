...
- **Python library** — creates a `{name}_lib/` package in `docker/`. Ask only if they mention needing custom Python modules. If enabled, the pack must also get a pytest suite in `<pack>/tests/` covering the lib's deterministic logic — `generate_dpack()` does not scaffold it, so create it after generation (reference layout: `decision-packs/mmm/tests/`).
