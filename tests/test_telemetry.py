...
def test_one_shot_export_builds_the_tree(tmp_path):
    ...
    assert "boom" in bodies and "thinking about it" in bodies
...
def test_session_id_env_or_work_dir(tmp_path, monkeypatch):
    monkeypatch.delenv("DLAB_SESSION_ID", raising=False)
    assert telemetry.session_id(tmp_path / "dlab-poem-workdir-1") == "dlab-poem-workdir-1"
    monkeypatch.setenv("DLAB_SESSION_ID", "wf-abc")
    assert telemetry.session_id(tmp_path) == "wf-abc"
